# O que é um numpy: é uma biblioteca que oferece estruturas de dados não nativas do python com suporte
# Porque numpy: oferece desempenho e ferramentas superiores à biblioteca padrão
# O numpy trabalha como a ideia de vetores (arrays), conhecidos como ndarrays
# Com a diferença que ndarrays só trabalham com dados do mesmo tipo que lembram muito as listas do python.
# Outra vantagem: por trabalhar com um só tipo de dado, o ndarry tem um desempenho muito melhor se comparado com
import time
...
# end_time = time.time()
# elapsed_time = end_time - start_time
# print(f'A criação de um ndarray de 1 bilhão de elementos em int de 9 não sinalizado levou: {elapsed_time}')
...
# m_linha = numpy.sort(matriz, axis=0)
...
# 5 - Agora vamos preparar alguns ndarrays para serem representados oir gráficos:
...
# 6 - Agora podemos utilizar uma das várias bibliotecas para plotar gráficos
